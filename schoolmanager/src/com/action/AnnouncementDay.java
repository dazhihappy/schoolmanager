...
public class AnnouncementDay implements ActiveDateInterface{
	...
	public ArrayList<UserCount> getData(String time, int page) throws Exception {
		...
		SchoolNotice tableInfo1 = new SchoolNotice();
		SchoolNotice tableInfo2 = new SchoolNotice();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		for(int i = 0; i< num ;i++ ){
			calendar1.set(year, month, day+i, 0, 0, 0);
			calendar2.set(year, month, day+i+1, 0, 0, 0);
			tableInfo1.setReleaseTime(calendar1.getTime());
			tableInfo2.setReleaseTime(calendar2.getTime());
			...
			userCounts.add(new UserCount(sdf.format(calendar1.getTime()), (int)DaoImpl.GetSelectCount(SchoolNotice.class, query)));
		}	
		return userCounts;
	}
