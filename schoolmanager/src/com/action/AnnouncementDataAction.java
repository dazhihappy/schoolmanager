...
public class AnnouncementDataAction extends ActionSupport{
	private static final long serialVersionUID = -4112723440763299961L;
	private int page; //上一页——1;下一页——2;
	private String time;//时间
	private String type;//类型:月——month,日——day
	
	public void getAnnouncementDataMethord() throws Exception{
...
//		System.out.println("type:"+type);
		...
		if("month".equals(getType())){//初始化时传明年年份
			userCounts = new AnnoumcementMonth().getData(time,page);//传当年年份:yyyy
			...
			userCounts = new AnnouncementDay().getData(time,page);//传坐标轴最左边那天的日期:yyyy-MM-dd
			String names ="";
			String counts = "";
			for (int i = 0; i < userCounts.size(); i++) {
