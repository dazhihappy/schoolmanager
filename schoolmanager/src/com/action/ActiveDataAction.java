...
public class ActiveDataAction extends ActionSupport{
	...
	private String type;//类型:时——hour,月——month,日——day
...
//		System.out.println("type:"+type);
		...
		String data=""; //存储数据
		if("hour".equals(getType())){//初始化时传明天日期
			userCounts = new ActiveHour().getData(time,page);//传当天日期:yyyy-MM-dd
		...
		}else if("month".equals(getType())){//初始化时传明年年份
			userCounts = new ActiveMonth().getData(time,page);//传当年年份:yyyy
			for (int i = 0; i < userCounts.size(); i++) {
				data += userCounts.get(i).getCount()+",";
				//System.out.println(userCounts.get(i).getName()+" : "+userCounts.get(i).getCount());
			}
		}else{//初始化时传当天日期+1
			userCounts = new ActiveDay().getData(time,page);//传坐标轴最左边那天的日期:yyyy-MM-dd
			String names ="";
			String counts = "";
			for (int i = 0; i < userCounts.size(); i++) {
