  ...
  <body>
  	...
  	<div class="alert alert-info">
		...
		<p align="center">提示! 初始显示今年的数据.如果月份还没到，则数据显示为0</p>
  		...
  		<button class="btn btn-info" type="button" onclick="location.href='announcementbyday.jsp'">日发布</button>
  		<button class="btn btn-info" type="button" onclick="location.href='announcementbymonth.jsp'">月发布</button>
  	</div>
    <div id="container" style="min-width:800px;height:400px"></div><br>
    <div align="center">
    	 <button class="btn btn-success" type="button" id="before">前一年</button>&nbsp;&nbsp;
    	 <button class="btn btn-success" type="button" id="after">后一年</button>
    </div>
		...
		$("#announcement").bind("click",function(){
			window.location.href="announcementbyday.jsp";
  			...
  			$("#before").bind("click",function(){
  				var year = $("#year").text();
  				$.get("getNoticeData",{"page":1,"time":year,"type":"month"},function(data){  			
  			...
  			$("#after").bind("click",function(){
  				var year = $("#year").text();
  			  	$.get("getNoticeData",{"page":2,"time":year,"type":"month"},function(data){  			
	  				var chart = $('#container').highcharts();
	  				var series=chart.series;            
	               	while(series.length > 0) {
	                   series[0].remove(false);
